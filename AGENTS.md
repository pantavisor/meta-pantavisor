...
| `docs/overview/` | **Reference for the meta-pantavisor layer itself** — architecture, build system, and build guide as flat `overview/*.md`, with `examples/`, `testing/` (split into `automated/` for building the pvtest distro — the suite itself is documented in pantavisor — and `manual/` for hand-driven testing plus `manual/testplans/`), and `ci/` as subdirectories |
...
- [docs/overview/testing/automated/index.md](docs/overview/testing/automated/index.md) — building the pvtest distro and its container fixtures, and the pantavisor pin (`PANTAVISOR_SRCREV`). **Using** pvtest is documented in the pantavisor repo, under `docs/testing/`: the harness (`pvtest-harness.md`), running and authoring (`appengine.md`), real devices (`device.md`), the native runner (`native.md`) and the test list (`pvtest-list.md`)
...
- **pvtest layout**: pvtest lives entirely in **pantavisor**, under `pvtest/` — the tester half (`pvtest-run.in`, `utils.in`, `common.in`; built with `PANTAVISOR_PVTEST=ON`, shipped by the `pantavisor-pvtest` package), the host half (`test.docker.sh`, `test.native.sh`, `host-common`, `device.txt`, the three READMEs), and the suites (`local/`, `remote/`). This layer only *assembles* it, and owns the `pv-example-*` container fixtures: `pantavisor_git.bb:do_deploy` stages all of it out of the pantavisor source tree, and `pantavisor-appengine-distro.bb` packs it into the tarball. Two consequences: a pvtest change is a pantavisor PR, not one here; and because `do_deploy` reads `${S}`, a `devtool modify pantavisor` build tests that checkout's suites. The deployed tarball layout is unchanged: `local/` and `remote/` sit at the root. Never source `utils` on the host, it shadows `pvcontrol`/`pventer`/`pvcurl`/`pvtx`. `host-common` is the *host-only* half shared by `test.docker.sh` and `test.native.sh` (test selection, tarball install, device manifest, ctrl protocol, summary) — not to be confused with `common`, which is shared with the tester half.
- **pvtest container fixtures**: the `pv-example-*` containers the suites run against are recipes **here**, in `recipes-containers/pv-examples/`, listed in `PV_PVTEST_CONTAINERS` / `PV_PVTEST_CONTAINERS_XCONNECT` in `pantavisor-appengine-distro.bb` — that list drives both `do_create_tarball[depends]` and the staging into `targets/`. So a new test container is a change here, alongside the pantavisor PR carrying the test that uses it. The same directory also holds the manual-testplan containers and `pv-avahi`/`pv-avahi-browse`, shipped product containers two xconnect tests happen to use. pantavisor is pulled in once, as recipe source (`PANTAVISOR_SRCREV` in `pantavisor.inc`).
- **pvtest native runner**: `test.native.sh` runs the tester on the host instead of in a container, for real devices only (`--device` required; the appengine pool stays on `test.docker.sh` because its targets *are* containers). It relies on two pantavisor overrides — `PVTEST_LIBDIR` (where `utils`/`common` live) and `PVTEST_ROOT` (prefix stripped to form a test id) — with a user+mount-namespace fallback for runners predating them. The per-target tarball bind mount is replaced by a symlink farm in the workspace, so `-o` still writes goldens back to the source tree. Ships as the slim `pantavisor-pvtest-scripts-<machine>.tar.gz` beside the full distro tarball. Docs: pantavisor's `docs/testing/native.md`.
