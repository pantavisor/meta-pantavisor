...
if test "${pv_config_size}" = "800"; then
	...
	setexpr pv_mmcdata ${distro_bootpart} + 1
fi

echo "pv_mmcdata selected = ${pv_mmcdata}"
...
load ${devtype} ${pv_mmcdev}:${distro_bootpart} ${envloadaddr} pv.env; setenv pv_env_size ${filesize}
echo Loading pv.env
...
if env exists pv_try; then
	if env exists pv_trying && test ${pv_trying} = ${pv_try}; then
		...
		save ${devtype} ${pv_mmcdev}:${distro_bootpart} ${envloadaddr} pv.env ${pv_env_size}
