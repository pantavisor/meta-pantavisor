...
`pv-manifest-strict` implies `pv-manifest-audit`. Setting both is fine — strict wins.
...
In this layer, references live in a `pv-manifests/` directory next to the
recipe (`recipes-pv/images/pv-manifests/` for the initramfs,
`dynamic-layers/virtualization-layer/recipes-pv/images/pv-manifests/` for the
appengine); downstream `.bbappend`s conventionally use `files/`. Either way
they are pulled into `${WORKDIR}` via `SRC_URI`:
...
the boundary unambiguous. The `-${DISTRO_CODENAME}` segment is dropped when
`DISTRO_CODENAME` is unset.
...
2. The build prints a full-add patch to the bitbake log and writes
   `${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.manifest.patch` (also copied to the final
   `deploy/images/<MACHINE>/` directory).
3. Apply the patch under your layer's reference directory (`pv-manifests/`
   in this layer, `files/` in a downstream bbappend):
