...
1. [The pvtest Harness](automated/index.md) — architecture, assignment models, execution flow,
   the readiness fence, workspace layout, how to read results, and the `test.docker.sh` flags
   reference
2. [Running and Authoring pvtests](automated/appengine.md) — running against the appengine
   pool, adding and updating tests, the authoring rules, and how CI drives it
3. [Running Against a Real Device](automated/devices.md) — the device manifest, the re-type
