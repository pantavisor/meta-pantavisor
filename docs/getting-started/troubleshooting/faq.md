...
To change Pantavisor-level behaviour (restart policy, auto-recovery, environment variables), edit the container's `run.json` directly in the checkout directory. See [Configure Applications](/meta-pantavisor/getting-started/develop/application/configure).
...
- The endpoint is not bound to localhost only — on images where it is, expose it
  ...
  `"listen": "0.0.0.0"` (see [Local Network](/meta-pantavisor/getting-started/operate/device-access/local-network))
...
Full reference: [pvr CLI](/meta-pantavisor/getting-started/develop/cli-tools/pvr-cli).
