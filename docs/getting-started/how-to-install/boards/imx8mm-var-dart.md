...
Variscite's production bootloader already self-enters SDP mode, so no separate
recovery build is needed — the bundle is built straight from the main image.
