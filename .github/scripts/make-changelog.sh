...
#     date is today. Downloads use predicted URLs based on the previous tag.
#     After writing the file, the script commits it with
...
#                                        # stable "## v<MAJOR>" section, commit "changelogs(<MAJOR>):
#                                        # finalize <MAJOR> changelog". <MAJOR> must not be tagged yet.
#
...
CHANGELOG_S3_URL_BASE="${CHANGELOG_S3_URL_BASE:-https://pantavisor-ci.s3.amazonaws.com/meta-pantavisor/changelog}"
