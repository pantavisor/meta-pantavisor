...
do_compile() {
    ...
    # Set up signing keys from pvr-ca
    ...
    # Keys are extracted to PVR_CONFIG_DIR
    ...
    ls -la ${PVR_CONFIG_DIR}/ || true

    cd ${B}/pvrrepo
    pvr init

    # Add device.json to the state (must be named device.json)
    cp -f ${WORKDIR}/device.json.ipam-example device.json

    pvr add
    pvr commit

    # Sign the device.json
